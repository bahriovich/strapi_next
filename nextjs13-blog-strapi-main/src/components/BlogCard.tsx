...
const BlogCard = ({ blog }: any) => {
  ...
  const textArray = description.map((elem: any) => {
    const childrenArray = elem.children || [];

    const text = childrenArray.map((child: any) => child.text).join(" ");
  ...
  const truncateBlogDesc =
    textArray.toString().length > 80
      ? textArray.toString().substring(0, 80) + "..."
      : textArray.toString();
